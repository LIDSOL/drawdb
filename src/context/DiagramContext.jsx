...
export default function DiagramContextProvider({ children }) {
  ...
  const deleteField = (field, tid, addToHistory = true) => {
    console.debug('[DiagramContext] deleteField called', { field, tid, addToHistory });
    ...
    console.debug('[DiagramContext] deleteField computed affected relationships and subtype snapshots', {
      directlyAffectedRelationships: directlyAffectedRelationships.map(r => ({ id: r.id, startTableId: r.startTableId, endTableId: r.endTableId, startFieldId: r.startFieldId, endFieldId: r.endFieldId })),
      compositePKAffectedRelationships: compositePKAffectedRelationships.map(r => r.id),
      compositeFKAffectedRelationships: compositeFKAffectedRelationships.map(r => r.id),
      subtypeChildTablesSnapshotKeys: Object.keys(subtypeChildTablesSnapshot),
    });
    ...
    if (addToHistory) {
      ...
      console.debug('[DiagramContext] deleteField pushed undo', {
        tid,
        fieldId: field.id,
        deletedRelationshipIds: affectedRelationships.map(r => r.id),
        childFieldsSnapshotTables: Object.keys(childFieldsSnapshot),
      });
    }
    ...
    console.debug('[DiagramContext] deleteField finished updates for table', { tid, resultingTableFields: updatedTables[tid].fields.map(f => ({ id: f.id, name: f.name, foreignK: f.foreignK })) });
  };

  const addRelationship = (relationshipData, autoGeneratedFkFields, childTableIdForFks, addToHistory = true) => {
    if (addToHistory) {
      console.debug('[DiagramContext] addRelationship (ADD) called', { relationshipData, autoGeneratedFkFieldsLength: (autoGeneratedFkFields || []).length, childTableIdForFks });
      ...
      console.debug('[DiagramContext] addRelationship pushed undo', { newRelationshipId });
    } else {
      let relationshipToInsert = { ...relationshipData };
      console.debug('[DiagramContext] addRelationship (RESTORE) called', { relationshipData, autoGeneratedFkFields, childTableIdForFks });
      ...
      if (Array.isArray(autoGeneratedFkFields) && autoGeneratedFkFields.length > 0 && typeof childTableIdForFks === 'number') {
        ...
        if (childTable) {
          const existingLen = childTable.fields.length;
          // Build mapping from original auto-generated field id -> new id
          const idMap = {};
          const newFields = autoGeneratedFkFields.map((f, idx) => {
            const newId = existingLen + idx;
            idMap[f.id] = newId;
            // ensure we don't accidentally keep stale ids
            return { ...f, id: newId };
          });

          console.debug('[DiagramContext] addRelationship building idMap for restored FK fields', { childTableIdForFks, existingLen, idMapPreview: Object.assign({}, idMap) });

          const updatedFields = [...childTable.fields, ...newFields].map((fld, i) => ({ ...fld, id: i }));

          // Apply updated fields to the table
          setTables((prev) => prev.map((t) => (t.id === childTableIdForFks ? { ...t, fields: updatedFields } : t)));

          // Remap relationship field ids to point to the newly inserted fields
          if (relationshipToInsert.endFieldId !== undefined && idMap[relationshipToInsert.endFieldId] !== undefined) {
            relationshipToInsert.endFieldId = idMap[relationshipToInsert.endFieldId];
          }
          if (relationshipToInsert.startFieldId !== undefined && idMap[relationshipToInsert.startFieldId] !== undefined) {
            relationshipToInsert.startFieldId = idMap[relationshipToInsert.startFieldId];
          }
          if (Array.isArray(relationshipToInsert.endFieldIds)) {
            relationshipToInsert.endFieldIds = relationshipToInsert.endFieldIds.map((fid) => (idMap[fid] !== undefined ? idMap[fid] : fid));
          }
          if (Array.isArray(relationshipToInsert.startFieldIds)) {
            relationshipToInsert.startFieldIds = relationshipToInsert.startFieldIds.map((fid) => (idMap[fid] !== undefined ? idMap[fid] : fid));
          }
        }
        console.debug('[DiagramContext] addRelationship after remap', { relationshipToInsert });
      }

      setRelationships((prevRels) => {
        let tempRels = [...prevRels];
        const idCollision = tempRels.some((r) => r.id === relationshipToInsert.id);
        if (idCollision) {
  ...
  const deleteRelationship = (id, addToHistory = true) => {
    console.debug('[DiagramContext] deleteRelationship called', { id, addToHistory });
    ...
    if (addToHistory) {
      ...
      console.debug('[DiagramContext] deleteRelationship pushed undo', {
        relationshipId: relationshipToDelete.id,
        childTablesSnapshotKeys: Object.keys(allChildTablesFieldsBeforeFkDeletion),
      });
    }
